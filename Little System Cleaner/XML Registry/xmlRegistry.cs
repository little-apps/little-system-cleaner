...
namespace Little_System_Cleaner.Xml
{
    /// <summary>
    ...
    public class xmlRegistry
    {
        ...
        /// <summary>
        /// Deletes bad registry key
        /// </summary>
        /// <param name="p">Bad Registry Key</param>
        /// <param name="w">XML Writer class</param>
        /// <returns>True if key was saved & removed</returns>
        public bool deleteAsXml(BadRegistryKey p, xmlWriter w)
        {
            if (Utils.RegKeyExists(p.RegKeyPath))
                if (!this.saveAsXml(w, false, p.RegKeyPath, p.ValueName, p.Problem))
                    return false;
